...
## Bolt
那玩意就是標準的：
把老馬原廠問題也一起抄，然後再進一步增加問題。鋼件剛性不足會被亞鉛件壓凹導致半自動走火、擊錘比照MWS原廠的凸起會被槍機K凹，連發司牙和副扳機件修改過設計改用扭簧，後定緩衝彈簧設計錯誤整個後定片會往前跑尻彈匣，甚至導致挑匣，可以用GM的零件將亞鉛件替換補上
...
V2更新擊錘止擋方式，因此不剛需鋼框了(改成鋁框)，移除擊錘滾輪
